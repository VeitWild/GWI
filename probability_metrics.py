...
class Wasserstein_Distance(Prob_Metric_abstract):
    ...
    def calculate_distance(self, X_S, X_B, components=False):
        ...
        aver_trace_r = self.GM2.variance(
            X_B
        ).mean()  # this is a bit inefficient since this trace could be calculated in the coviarance matrix call and returned jointly
        ...
        eigenvalues = torch.linalg.eigvals(
            r_XSXB @ k_XBXS + 100 * torch.eye(N_S)
        ).real - 100 * torch.ones(N_S)
